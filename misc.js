...
class Misc { // Declaring export as a class because cbf to make other way work properly. Should probably do other way for consistancy though
    ...
    static getFetishes(id) {
        var user = data.people[m.author.id]
        var fetishes = user.fetishes || null
        if (fetishes) {
            var matches = Object.keys(data.people).filter(k => data.people[k].fetishes && data.people[k].fetishes[`${incoming[0]}`] == "likes")
            console.log(matches);
